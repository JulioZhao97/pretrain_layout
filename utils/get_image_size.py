...
def get_image_size(file_path, client=None):
    """
    Return (width, height) for a given img file content - no external
    dependencies except the os and struct builtin modules
    """
    img = get_image_metadata(file_path, client=client)
...
def get_image_metadata(file_path, client=None):
    """
    ...
    if not file_path.startswith("s3://"):
        ...
        img_bytes = client.get(file_path)
        buffer = io.BytesIO(img_bytes)
        input = io.BufferedReader(buffer)
        size = len(img_bytes)
        return get_image_metadata_from_bytesio(input, size)
